## level set without redistancing
# computes a level set without doing redistancing, therefore the function collapses
...
# path of the frames
path = 'images/level set without redistancing/image{}.png'
# path of the gif
path_gif = 'images/level set without redistancing/out.gif'
...
def redistancing(phi):
    # return np.sign(phi) * (1-norm(grad(phi)))
...
#frame count
frame_draw = [int(i*n_iter/frames) for i in range(frames+1)]
fr_count = 0
for i in range(n_iter):
    ...
    for k,x_i in enumerate(x):
        for j,y_j in enumerate(y):
            intersection[k,j] = intersect(phi[k,j],0,0.1)
